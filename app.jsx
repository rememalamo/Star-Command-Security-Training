...
/* ============ Cloud sync (Firebase Auth + Firestore) -- optional ============
   If window.FIREBASE_CONFIG is filled in (see firebase-config.js), the game
   uses Google sign-in and saves progress to the cloud so it follows you across
   devices. If it is left as placeholders, the game runs local-only as before. */
const Cloud = (() => {
  let ready = false, auth = null, db = null;
  const cfg = (typeof window !== "undefined" && window.FIREBASE_CONFIG) ? window.FIREBASE_CONFIG : null;
  const placeholder = (v) => !v || (typeof v === "string" && v.indexOf("YOUR_") === 0);
  const isConfigured = !!(cfg && !placeholder(cfg.apiKey) && !placeholder(cfg.projectId));
  ...
  return {
    configured: () => isConfigured && hasSdk(),
    onAuth: (cb) => { if (init()) auth.onAuthStateChanged(cb); },
    signIn: () => { if (init()) { const p = new firebase.auth.GoogleAuthProvider(); return auth.signInWithPopup(p); } return Promise.reject(new Error("cloud unavailable")); },
    signOut: () => (auth ? auth.signOut() : Promise.resolve()),
    load: (uid) => {
      if (!ready || !uid) return Promise.resolve(null);
      return db.collection("starCommandProgress").doc(uid).get()
        .then((d) => (d.exists ? d.data() : null)).catch(() => null);
    },
    save: (uid, prog) => {
      if (!ready || !uid) return Promise.resolve();
      return db.collection("starCommandProgress").doc(uid).set(prog).catch(() => {});
    },
  };
})();

/* Merge two progress objects, keeping the best of each. Used on first sign-in so
   local progress is preserved and pushed up, not overwritten. */
const mergeProg = (a, b) => {
  a = a || {}; b = b || {};
  const out = { ...DEFAULT_PROG, ...a, ...b };
  ["mastered", "lessonsRead", "pbq", "boss", "certBoss"].forEach((k) => {
    const m = {}, oa = a[k] || {}, ob = b[k] || {};
    Object.keys(oa).concat(Object.keys(ob)).forEach((key) => {
      const va = oa[key], vb = ob[key];
      if (typeof va === "number" || typeof vb === "number") m[key] = Math.max(va || 0, vb || 0);
      else m[key] = va || vb;
    });
    out[k] = m;
  });
  ...
  out.zurgDefeated = !!(a.zurgDefeated || b.zurgDefeated);
  out.sound = (b.sound !== undefined) ? b.sound : (a.sound !== undefined ? a.sound : true);
  return out;
};
...
function SignInLanding({ onSignIn, onLocal, error }) {
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center",
      background: "radial-gradient(1200px 600px at 50% -10%, #3B2566 0%, #201640 55%, #140E2A 100%)",
      color: "#fff", fontFamily: "'Space Grotesk', system-ui, sans-serif", padding: 20 }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;700&family=IBM+Plex+Mono:wght@400;500;600&display=swap');`}</style>
      <div style={{ width: "100%", maxWidth: 460, textAlign: "center" }}>
        <div style={{ fontSize: 54, marginBottom: 6 }}>{"\uD83D\uDE80"}</div>
        <div className="mono" style={{ fontSize: 11, letterSpacing: ".3em", color: "#8FE388" }}>GALACTIC ALLIANCE | UNIVERSE PROTECTION UNIT</div>
        <div style={{ fontSize: 30, fontWeight: 700, marginTop: 8, lineHeight: 1.15 }}>STAR COMMAND</div>
        <div style={{ fontSize: 18, fontWeight: 500, color: "#C4B6E4", marginBottom: 18 }}>Security Ranger</div>
        <div style={{ fontSize: 14, lineHeight: 1.6, color: "#B9AEDA", marginBottom: 24 }}>
          Sign in to save your certification progress and continue on any device -- your phone, your laptop, anywhere in the galaxy.
        </div>
        <button onClick={onSignIn}
          style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, width: "100%",
            background: "#fff", color: "#1E2430", border: "none", borderRadius: 10, padding: "13px 16px",
        ...
        {error ? <div className="mono" style={{ fontSize: 11, color: "#E88", marginTop: 12 }}>{error}</div> : null}
        <div onClick={onLocal} className="mono"
          style={{ marginTop: 18, fontSize: 12, color: "#9C8FC6", cursor: "pointer", textDecoration: "underline" }}>
          Continue without signing in (this device only)
        </div>
        <div className="mono" style={{ marginTop: 26, fontSize: 10, color: "#6E639A", lineHeight: 1.7 }}>
          Progress syncs securely to your Google account.<br />
          No password is stored by Star Command.
        </div>
      </div>
    </div>
  );
...
function Splash({ label }) {
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center",
      background: "#140E2A", color: "#C4B6E4", fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, letterSpacing: ".2em" }}>
      {label || "LOADING STAR COMMAND..."}
    </div>
  );
}
...
function App() {
  ...
  const [authState, setAuthState] = useState(Cloud.configured() ? "loading" : "local");
  ...
  useEffect(() => {
    ...
    Cloud.onAuth((u) => {
      if (u) {
        setUser({ uid: u.uid, name: u.displayName || "Ranger", email: u.email || "", photo: u.photoURL || "" });
        Cloud.load(u.uid).then((cloudProg) => {
          setProg((local) => {
            const merged = mergeProg(local, cloudProg);
            Cloud.save(u.uid, merged);
            return merged;
          });
          setAuthState("in");
        });
  ...
  useEffect(() => {
    saveProg(prog); SFX.setEnabled(prog.sound);
    if (authState === "in" && user && user.uid) {
      if (saveTimer.current) clearTimeout(saveTimer.current);
      saveTimer.current = setTimeout(() => Cloud.save(user.uid, prog), 800);
    }
  }, [prog]);

  const update = (fn) => setProg((p) => fn({ ...p }));
  const doSignIn = () => { SFX.click(); setAuthError(""); Cloud.signIn().catch(() => setAuthError("Sign-in was cancelled or blocked. Please try again.")); };
  const doSignOut = () => { SFX.click(); Cloud.signOut().then(() => { setScreen({ view: "hub" }); }); };
  const playLocal = () => { SFX.click(); setAuthState("local"); };

  if (authState === "loading") return <Splash />;
  if (authState === "out") return <SignInLanding onSignIn={doSignIn} onLocal={playLocal} error={authError} />;
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 16, alignItems: "center" }}>
          ...
          {authState === "in" && user ? (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {user.photo ? <img src={user.photo} alt="" style={{ width: 26, height: 26, borderRadius: "50%" }} referrerPolicy="no-referrer" /> : null}
              <div style={{ textAlign: "right" }}>
                <div className="mono" style={{ fontSize: 10, color: "#C4B6E4" }}>SIGNED IN</div>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#8FE388" }}>{(user.name || "Ranger").split(" ")[0]}</div>
              </div>
              <button onClick={doSignOut}
                style={{ background: "none", border: "1px solid #6A5A94", color: "#C4B6E4", borderRadius: 8, padding: "6px 10px", cursor: "pointer", fontSize: 11 }}
