...
window.PACKS.__core = {
  zurg: {
    ...
    stages: [
      ...
      { tag: "SC-300", sit: "The PowerShell host stole a session token. Zurg's agents are now signing in as Ranger d.okafor from three continents -- MFA shows 'previously satisfied.'",
        e: "SigninLogs: d.okafor | Dallas 09:02 [OK] | Trade World proxy 09:31 (token reuse)\nCA policy: MFA required -- satisfied by existing session",
        options: [
  ...
  certTrials: {
    sc200: {
      title: "Cert Trial: XL's Final Onslaught",
      brief: "Before Star Command certifies you as a Security Operations Analyst, the shape-shifting hulk XL mounts one last onslaught spanning every SC-200 discipline -- environment, incident response, and threat hunting, all at once.",
      win: "XL's onslaught breaks against a SOC that sees everything, responds instantly, and hunts him down. SC-200 certified at 100% -- rank up, Ranger!",
      lose: "The onslaught found a seam this time. Re-drill your weakest SecOps discipline and run the gauntlet again.",
      stages: [
        { tag: "Environment", sit: "XL scatters related alerts across endpoint, identity, and email to fracture your view.",
          e: "Separate alerts in Defender for Endpoint, Identity, and Office 365 -- clearly one attack",
          options: [
            { t: "Investigate the single correlated Defender XDR incident holistically", d: 0, r: 4, ev: 0, ql: "best", w: "XDR correlation reveals the full multi-domain attack chain -- the unified view XL fears." },
            { t: "Chase each product's alert in isolation", d: 16, r: -5, ev: 0, ql: "bad", w: "Siloed alert-chasing is the blind spot; correlation is the point of XDR." },
            { t: "Dismiss them as unrelated", d: 18, r: -6, ev: 0, ql: "bad", w: "Dismissing correlated attack signals lets the intrusion run." },
          ]},
        { tag: "Incident Response", sit: "An endpoint shows active ransomware behavior, spreading fast.",
          e: "MDE alert: ransomware encrypting files, lateral movement beginning",
          options: [
            { t: "Isolate the device first (contain), then collect the investigation package and scan in the contained state", d: 0, r: 4, ev: 1, ql: "best", w: "Isolate to stop lateral spread, then investigate -- the correct MDE response order." },
            { t: "Run a full antivirus scan before isolating", d: 14, r: -4, ev: 0, ql: "bad", w: "Scanning first lets ransomware keep spreading; isolate to contain first." },
            { t: "Reimage immediately with no evidence collection", d: 12, r: -3, ev: -8, ql: "ok", w: "Reimaging before collecting the investigation package destroys the evidence trail." },
          ]},
        { tag: "Threat Hunting", sit: "XL hides in subtle telemetry. You must proactively hunt across the right data.",
          e: "Hypothesis: credential theft followed by lateral movement; need the right tables and KQL",
          options: [
            { t: "Write targeted KQL across the correct Advanced Hunting tables (identity, device, network) to test the hypothesis", d: 0, r: 4, ev: 0, ql: "best", w: "Hypothesis-driven KQL over the right tables is how proactive hunting surfaces a hidden adversary." },
            { t: "Wait for an analytics rule to alert instead of hunting", d: 12, r: -3, ev: 0, ql: "ok", w: "Hunting is proactive by design; waiting cedes the initiative to XL." },
            { t: "Query random tables with no hypothesis", d: 14, r: -4, ev: 0, ql: "bad", w: "Unfocused querying wastes time; hunting is hypothesis-driven." },
          ]},
        { tag: "Automation", sit: "XL strikes faster than humans can react across many incidents at once.",
          e: "Confirmed incidents need instant assignment, tagging, and isolate-machine + disable-user response",
          options: [
            { t: "Use automation rules to orchestrate incidents and trigger a playbook that isolates and disables", d: 0, r: 4, ev: 0, ql: "best", w: "Automation rules orchestrate; the playbook responds -- SOAR at machine speed." },
            { t: "Handle every incident manually", d: 12, r: -3, ev: 0, ql: "ok", w: "Manual-only can't match XL's pace; automation is the equalizer." },
            { t: "Build a scheduled analytics rule to perform the response", d: 14, r: -4, ev: 0, ql: "bad", w: "Analytics rules detect; response is a playbook's job." },
          ]},
        { tag: "Full-spectrum", sit: "Cornered, XL bets you can't prove coverage or tune the noise he created.",
          e: "Coverage gaps unknown; a key detection is drowning in false positives",
          options: [
            { t: "Map detections to MITRE ATT&CK, use SOC optimization to close gaps, and tune the noisy rule with exclusions (keep it enabled)", d: 0, r: 4, ev: 1, ql: "best", w: "Visible coverage plus tuned detection leaves XL no blind spot to hide in." },
            { t: "Disable the noisy rule and assume coverage is fine", d: 18, r: -6, ev: 0, ql: "bad", w: "Disabling creates the exact blind spot XL wants; assumptions aren't coverage." },
            { t: "Ingest every log at max retention and call it covered", d: 8, r: -2, ev: 0, ql: "ok", w: "More data isn't more coverage; optimization tunes gaps and spend together." },
          ]},
      ],
    },
    sc300: {
